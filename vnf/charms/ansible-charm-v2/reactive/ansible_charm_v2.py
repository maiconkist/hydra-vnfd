...
@when('ansible-charm-v2.configured')
@when('actions.ansible_hydra_server')
def hydra_server():
    try:
        # edit ansible hosts file with the VNF parameters
        h = open("/etc/ansible/hosts", "wt")
        ...
        c.close()
        # execute the ansible playbook
        path = find('ansible_hydra_server.yaml','/var/lib/juju/agents/')
        call = ['ansible-playbook', path]
        subprocess.check_call(call)
        ...
        remove_flag('actions.ansible_hydra_server')
    finally:
        remove_flag('actions.ansible_hydra_server')

@when('ansible-charm-v2.configured')
@when('actions.ansible_hydra_client_2tx_2rx')
def hydra_client_2tx_2rx():
    try:
        # edit ansible hosts file with the VNF parameters
        h = open("/etc/ansible/hosts", "wt")
        h.write("[targets]\n")
        ...
        c.close()
        # execute the ansible playbook
        path = find('ansible_hydra_client_2tx_2rx.yaml','/var/lib/juju/agents/')
        call = ['ansible-playbook', path]
        subprocess.check_call(call)
        status_set('active', 'ready!')
    except Exception as e:
        ...
        remove_flag('actions.ansible_hydra_client_2tx_2rx')
    finally:
        remove_flag('actions.ansible_hydra_client_2tx_2rx')


@when('ansible-charm-v2.configured')
@when('actions.ansible_hydra_rx1')
def hydra_rx1():
    try:
        # edit ansible hosts file with the VNF parameters
        h = open("/etc/ansible/hosts", "wt")
        ...
        c.close()
        # execute the ansible playbook
        path = find('ansible_hydra_rx1.yaml','/var/lib/juju/agents/')
        call = ['ansible-playbook', path]
        subprocess.check_call(call)
        status_set('active', 'ready!')
    except Exception as e:
        ...
        remove_flag('actions.ansible_hydra_rx1')
    finally:
        remove_flag('actions.ansible_hydra_rx1')

# Function to find the playbook path
@when('ansible-charm-v2.configured')
@when('actions.ansible_hydra_rx2')
def hydra_rx2():
    try:
        # edit ansible hosts file with the VNF parameters
        h = open("/etc/ansible/hosts", "wt")
        ...
        c.close()
        # execute the ansible playbook
        path = find('ansible_hydra_rx2.yaml','/var/lib/juju/agents/')
        call = ['ansible-playbook', path]
        subprocess.check_call(call)
        status_set('active', 'ready!')
    except Exception as e:
        ...
        remove_flag('actions.ansible_hydra_rx2')
    finally:
        remove_flag('actions.ansible_hydra_rx2')
